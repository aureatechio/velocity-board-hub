...
export const SpeedMeter = ({ level, label }: SpeedMeterProps) => {
  ...
  return (
    <div className="w-full">
      ...
      <div className="relative h-8 rounded-full bg-gradient-to-r from-destructive via-warning to-success overflow-hidden">
        <motion.div
          initial={{ left: "50%" }}
          animate={{ left: current.position === "left-[10%]" ? "10%" : current.position === "left-[50%]" ? "50%" : "90%" }}
          transition={{ duration: 0.5, type: "spring" }}
          className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-6 h-6 bg-foreground rounded-full border-2 border-background shadow-lg"
        />
      </div>
